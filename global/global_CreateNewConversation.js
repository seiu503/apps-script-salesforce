async function global_createNewConversation(
  // Campaign_Assessment__c = 'a2RRt000000pii9MAA', 
  // External_Organizer_MLDP__c = '0054N000003rixvQAA',
  // Source__c = 'Tabling',
  // Assessment__c = '2',
  // Notes__c = 'Notes test',
  ...
  console.log(`global_createNewConversation.gs > 18}`);
  console.log(body);
  ...
  if (body) {
    try {
      ...
      try {
        await global_getCAById(Campaign_Assessment__c, env, campaign);
      } catch {
        logErrorFunctions('global_createNewConversation', {body}, '', err);
        return {
            Success: false,
