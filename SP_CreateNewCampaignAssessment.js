async function SP_createNewCampaignAssessment(
  ...
  // Potential_Leader__c,
  // AppSheet_ID__c,
  // Signed_OMA__c, // placeholder
  // Signed_OMA_Date__c, // placeholder
  // ) {
  ...
  if (cleanBody) {
    try {
      ...
      if (rowIndex > 1) {
        const cell = spWorkers.getRange(`A${rowIndex + 2}`); // add 2 to row index bc there was no Appsheet ID in header row
        cell.setValue(id);
      }
